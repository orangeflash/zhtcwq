...
Page({
  ...
  onLoad: function (options) {
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        if (res.data.address.length > 14) {
          res.data.address = '活动地址：' + res.data.address.substring(0, 14) + '...'
        }
        else {
          res.data.address = '活动地址：' + res.data.address
        }
        if (res.data.title.length > 9) {
        ...
        wx.downloadFile({
          url: options.url+res.data.logo, //仅为示例，并非真实的资源
          success: function (res) {
            console.log(res)
            that.setData({
        ...
        //         // 二维码的图片
        //         var logo1 = res.data
        //         console.log('商家的logo', logo,'小程序码logo1',logo1)
        //         wx.downloadFile({
  ...
  createNewImg: function () {
    ...
    var name = that.data.QgGoodInfo.title, money = '￥' + that.data.QgGoodInfo.money, price = '￥' + that.data.QgGoodInfo.price, title = that.data.QgGoodInfo.money + '元报名' + that.data.QgGoodInfo.title, endtime = "活动结束日期:" +that.data.QgGoodInfo.end_time;
    //绘制名字
    context.setFontSize(24);
    context.setFillStyle('#000000');
    context.setTextAlign('center');
    context.fillText(name, 187.5, 50,340);
    context.stroke();
    ...
    context.setFontSize(16);
    context.setTextAlign('center');
    context.fillText("长按二维码识别小程序参与活动", 187.5, 529);
    //
    ...
    context.setFontSize(13);
    context.setTextAlign('center');
    context.fillText(that.data.QgGoodInfo.address, 187.5, 605);
